...
class UserItem(Resource):
    def get(self, userID):
        ...
        body.add_control_edit_user(
            userID, url_for("api.useritem", userID=userID))
        body.add_control_delete_user(
            userID, url_for("api.useritem", userID=userID))
        body.add_control_get_bookables_by(userID=userID, url=url_for(
            "api.booakblecollectionofuser", userID=userID))
        body.add_control_get_all_bookables(
            userID=userID, url=url_for("api.bookablecollection", userID=userID))
...
class BooakbleCollectionofUser(Resource):
    def get(self, userID):
        ...
        body.add_control("self", url_for(
            "api.booakblecollectionofuser", userID=userID))
        body.add_control_add_bookable(userID=userID, url=url_for(
            "api.booakblecollectionofuser", userID=userID))
...
class BookableCollection(Resource):
    def get(self, userID):
        ...
        for i in bookables:
            ...
            item.add_control("self", url_for(
                "api.bookablecollection", userID=userID))
            body["items"].append(item)
        body.add_namespace("bookingmeta", LINK_RELATIONS_URL)
        body.add_control("self", url_for(
            "api.bookablecollection", userID=userID))
...
class SlotCollectionofUser(Resource):
    def get(self, userID, bookableID):
        ...
        for i in slots:
            ...
            item.add_control("self", url_for(
                "api.slotitemofuser", userID=userID, bookableID=bookableID, slotID=i.id))
            body["items"].append(item)

        body.add_namespace("bookingmeta", LINK_RELATIONS_URL)
        body.add_control("self", url_for(
            "api.slotcollectionofuser", userID=userID, bookableID=bookableID))
        body.add_control_add_slot(userID=userID, bookableID=bookableID, url=url_for(
            "api.slotcollectionofuser", userID=userID, bookableID=bookableID))
        body.add_control_bookable(userID=userID, bookableID=bookableID, url=url_for(
            "api.bookableitemofuser", userID=userID, bookableID=bookableID))
        return Response(json.dumps(body), 200, mimetype=MASON)
